...
def register_exception_handlers(app: FastAPI) -> None:
    ...
    @app.exception_handler(AuthenticationDomainError)
    async def handle_authentication_domain_error(_: Request, exc: AuthenticationDomainError) -> JSONResponse: # pyright: ignore[reportUnusedFunction]
        return _response(status.HTTP_401_UNAUTHORIZED, str(exc))

    @app.exception_handler(AuthorizationDomainError)
    async def handle_authorization_domain_error(_: Request, exc: AuthorizationDomainError) -> JSONResponse: # pyright: ignore[reportUnusedFunction]
