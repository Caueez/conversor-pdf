...
from account_service.api.schemas import TokenPairResponse, UserResponse
from account_service.application.schemas import CreateUserDTO, SessionTokenPairDTO, UserDTO
...
def test_dto_to_token_pair_response_maps_expected_fields() -> None:
    token_dto = SessionTokenPairDTO(
        access_token="access",
    ...
    response = dto_to_token_pair_response(token_dto)

    assert isinstance(response, TokenPairResponse)
    assert response.model_dump() == token_dto.model_dump()
