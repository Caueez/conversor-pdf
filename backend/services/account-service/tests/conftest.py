...
async def _truncate_tables(postgres_dsn: str) -> None:
    ...
    try:
        await connection.execute(
            "TRUNCATE TABLE users, sessions, role_permissions, permissions, roles RESTART IDENTITY CASCADE;"
        )
    finally:
...
@pytest_asyncio.fixture
async def session_repo(db_backend: Postgres, clean_database: None) -> PostgresSessionRepoAdapter:
    return PostgresSessionRepoAdapter(db_backend)
